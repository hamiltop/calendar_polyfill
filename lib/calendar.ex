if !Code.ensure_loaded?(Calendar) do
  ...
  defmodule Date do
    ...
    def from_iso8601(<<year::4-bytes, ?-, month::2-bytes, ?-, day::2-bytes>>) do
      res = with {year, ""}  <- Integer.parse(year),
  ...
  defmodule Time do
    ...
    def from_iso8601(<<hour::2-bytes, ?:, min::2-bytes, ?:, sec::2-bytes, rest::binary>>) do
      res = with {hour, ""}       <- Integer.parse(hour),
  ...
  defmodule DateTime do
    ...
    def from_unix(integer, unit \\ :seconds) when is_integer(integer) and integer >= 0 do
      total = System.convert_time_unit(integer, translate_time_unit(unit), translate_time_unit(:microseconds))
    ...
    def precision_for_unit(unit) do
      subseconds = div System.convert_time_unit(1, :seconds, translate_time_unit(unit)), 10
    ...
    def to_unix(%DateTime{calendar: Calendar.ISO, std_offset: std_offset, utc_offset: utc_offset,
      ...
      seconds =
        ...
        |> Kernel.-(utc_offset)
        |> Kernel.-(std_offset)
      System.convert_time_unit((seconds - @unix_epoch) * 1_000_000 + microsecond, translate_time_unit(:microseconds), translate_time_unit(unit))
    end
